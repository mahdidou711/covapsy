...
def angle_deg_to_duty(angle_deg: float) -> float:
    ...
    Valeurs calibrées :
      centre = 7.90 %  (roues droites)
      min    = 6.20 %  (butée gauche, -18°)
      max    = 9.60 %  (butée droite, +18°)
      débattement = ±18°
    """
    ...
    # Le centre (7.90) n'est pas au milieu de [6.20, 9.60], donc la course
    # gauche (1.70 %) et droite (1.70 %) sont interpolées séparément.
    # x=+1 → 7.90 + 1×(9.60-7.90) = 9.60  (butée droite)
    # x= 0 → 7.90                           (centre)
    # x=-1 → 7.90 + (-1)×(7.90-6.20) = 6.20 (butée gauche)
    if x >= 0:
